...
if __name__ == "__main__":
    ...
    fs1, signal1 = wavfile.read("./dataset/original/Traffic_Noise2.wav")
    fs2, signal2 = wavfile.read("./dataset/original/Traffic_Noise.wav")
    fs3, signal3 = wavfile.read("./dataset/original/살려주세요_cropped.wav")
    signal3 = signal3*0.5
    ...
    signals = cut_signals([signal1, signal2, signal3])
    ...
    source1_loc = np.array([10, 50+10])
    source2_loc = np.array([5, 50-2.5])
    source3_loc = np.array([50+45, 50])
    ...
    mics = pra.circular_2D_array(center=mic_center, M=30, phi0=0, radius=mic_radius)
    angle = 90
    ...
    mics = pra.Beamformer(mics, room.fs, N=fft_len, Lg=Lg)
    ...
    sf.write('./results/2d/beamforming/20220315_all_mix.wav', room.mic_array.signals[-1,:].astype(np.int16),  Fs)

    plot = True
    # room.mic_array.rake_mvdr_filters(room.sources[2][:2], room.sources[1][:2], sigma2_n * np.eye(mics.Lg * mics.M), epsilon=0.005)
    # room.mic_array.rake_mvdr_filters(room.sources[1][:2], room.sources[0][:2], sigma2_n * np.eye(mics.Lg * mics.M), epsilon=0.005)
    room.mic_array.rake_delay_and_sum_weights(room.sources[1][:2])
    if plot:
        fig, ax = room.plot(freq=[500, 1000, 2000, 3000, 4000, 5000, 6000], img_order=0)
        ax.legend(['500', '1000', '2000', '3000', '4000', '5000', '6000'])
    ...
    # draw_spectogram(signal_das, fs, fft_size, fft_hop, analysis_window)
        ...
        sf.write(f'./results/2d/beamforming/20220315_{algoritm}_source{2}_frec{fc}.wav', signal_das_filtered.astype(np.int16), Fs)
