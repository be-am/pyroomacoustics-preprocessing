...
if __name__ == "__main__":
    ...
    fs1, signal1 = wavfile.read("./dataset/original/Boat_Sounds.wav")
    fs2, signal2 = wavfile.read("./dataset/original/ocean_wave_sound.wav")
    ...
    signals = cut_signals([signal1, signal2])


    # Create a 10X5X5 metres shoe box room
    room_dim=[200,200,200]
    room = pra.ShoeBox(
        room_dim,
    ...
    # 30 degrees
    source1 = np.array([130, 50, 1])
    source2 = np.array([50, 50, 5])
    ...
    sf.write('./results/3d/beamforming/ship/all_mix.wav', room.mic_array.signals[-1,:].astype(np.int16),  fs)

    plot = False

    for algoritm in bf_algoritms:
        if algoritm == 'das':
            room.mic_array.rake_delay_and_sum_weights(room.sources[0][:2], room.sources[1][:2])
        elif algoritm == 'mvdr':
            room.mic_array.rake_mvdr_filters(room.sources[0][:2], room.sources[1][:2], sigma2_n * np.eye(mics.Lg * mics.M), delay=0.0, epsilon=0.005)
        ...
        # draw_spectogram(signal_das, fs, fft_size, fft_hop, analysis_window)
            ...
            sf.write(f'./results/3d/beamforming/ship/{algoritm}_source0_frec{fc}.wav', signal_das_filtered.astype(np.int16), fs)
